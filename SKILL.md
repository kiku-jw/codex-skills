...
2. Choose the best source inputs and run the `session-to-post` CLI with explicit paths. Read `~/.codex/skills/session-to-post/README.md` for the current flag set and source-selection rules if needed, especially when deciding between repo diff, saved patch, notes, and transcript inputs.
