...
2. Launch the dispatcher with the exact script `~/.codex/skills/long-context-dispatch/scripts/long_context_dispatch.py`. Put the request in a temporary Markdown file, pass an explicit `--cwd`, `--reason`, and `--scope`, and keep the child job read-only.
