...
## Instructions
1. Inspect the target skill directory first. Read `SKILL.md`, only the references/scripts/assets actually needed for the rewrite, and `agents/openai.yaml` if it exists. Use `~/.codex/skills/.system/skill-creator/references/openai_yaml.md` when UI metadata needs to be created or checked.
