...
2. Browse the current landscape using primary sources first, with GitHub, official docs, pricing pages, and other relevant discovery sources. Use `~/.codex/skills/tool-scout/references/search-patterns.md` for search patterns and `~/.codex/skills/tool-scout/references/evaluation-matrix.md` for the comparison frame only when needed.
