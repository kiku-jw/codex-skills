...
## Instructions
1. Gate the decision first. Confirm what was chosen, what was rejected, and why this is worth preserving. If the choice is still fuzzy, route to `$product-council` or `$spec-bundle` instead. Read `~/.codex/skills/adr-log/references/storage-rules.md` only when you need the storage surface and `~/.codex/skills/adr-log/references/entry-template.md` only when you need the exact entry shape.
